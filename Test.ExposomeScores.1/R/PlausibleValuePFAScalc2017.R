...
PlausibleValuePFAScalc2017<-function(data,
  ...
  if (use.isomers) {
    ...
    set.seed(seed)
    fs.draw<-mirt::fscores(mod.mirt.isomer,response.pattern=data.cut,plausible.draws=ndraws)
    ...
    set.seed(seed)
    fs.draw<-mirt::fscores(mod.mirt.sum,response.pattern=data.cut,plausible.draws=ndraws)
